(function ($) {
    ...
    GetList.prototype = {
        ...
        BuildHead: function () {
            ...
            //总条数
            var $divListDataCount = $("<div class=\"list_datacount\">共找到<span id='span_datacount' style='color:red;'>0</span>个结果<div class=\"downloadico\"></div></div>");
        ...
        BuildList: function (obj, index, page, type, yzm, guid) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data == "remind") {
                        window.location.href = "/Html_Pages/VisitRemind.html";
                    }
                    datalist = eval("(" + data + ")");
                        ...
                        if (datalist.length > 1) {
                            ...
                            for (var i = 1; i < datalist.length; i++) {
                                ...
                                //+  "<a href='/content/contents?DocID=" + key+"'  target='_blank' style='color:Black; text-decoration:none;display:none'>" + title + "</a>"
                                                    + "<a href='#' onclick='javascript:Navi(\"" + key + "\",\"" + keyWords + "\")' target='_self' style='color:Black; text-decoration:none'>" + title + "</a>"
                                                    + "</div>"
                                                + "</td>"
                                            + "</tr>"
                                             + "<tr>"
